...
InsurancePolicy& Database::addNewPolicy(std::tuple<int, int, int> startDate, std::tuple<int, int, int> endDate, double cost, double payout, int type, bool status, int client, int employee)
{
	...
	if (get<0>(startDate) > 31 || get<0>(startDate) < 1 || get<1>(startDate) > 12 || get<1>(startDate) < 1 || get<2>(startDate) < 1970 || get<2>(startDate) > 2050)
		errors += "startDate: invalid value\n";
	if (get<0>(endDate) > 31 || get<0>(endDate) < 1 || get<1>(endDate) > 12 || get<1>(endDate) < 1 || get<2>(endDate) < 1970 || get<2>(endDate) > 2050)
	...
	policy->setStartDate(get<0>(startDate), get<1>(startDate), get<2>(startDate));
	policy->setEndDate(get<0>(endDate), get<1>(endDate), get<2>(endDate));
	policy->setCost(cost);
