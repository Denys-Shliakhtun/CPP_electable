	...
	while (!done)
	{
		...
		try
		{
			...
			case 6:
				...
				break;
			case 7:
				disableInsurance(database);
				break;
			case 8:
				addInsuranceCost(database);
				break;
			case 0:
				done = true;
				break;
...
int selection()
{
	std::cout << "\nInsurance Policy Database\n"
		...
		<< "7) Disable an insurance\n"
		<< "8) Add to cost of an insurance\n"
...
void disableInsurance(Database& database)
{
	int num;
	...
	database.getPolicyByNumber(num).setIsExpired();

	std::cout << "The policy #" << num << " set as expired successfully." << std::endl;
}

void addInsuranceCost(Database& database)
{
	int num;
	...
	int cost;
	std::cout << "\nEnter how much to increase cost: ";
	std::cin >> cost;

	cost += database.getPolicyByNumber(num).getCost();

	database.getPolicyByNumber(num).setCost(cost);

	std::cout << "The cost of the policy #" << num << " has been updated to: " << cost << std::endl;
}